...
export async function fetchCharacter(client, doll) {
  const html = await client.fetchRawHtml(
    '/game/index.php',
    { mod: 'overview', doll },
    { noXhr: true },
  );
  const snap = parseCharSnapshot(html);
  ...
  if (snap.doll !== null && snap.doll !== doll) {
    log.warn(`fetchCharacter(${doll}) returned active=${snap.doll} — ignoring`);
...
// derrubam o resto.
export async function fetchAllCharacters(client) {
  const tasks = [];
  for (let d = 1; d <= MAX_DOLLS; d++) {
    tasks.push(
      fetchCharacter(client, d).catch((e) => {
