...
export async function fetchPackages(client) {
  ...
  return { html, packages: parsePackages(typeof html === 'string' ? html : '') };
}
...
export async function movePackageToInventory(client, pkg, gridSnapshot) {
  ...
  if (!chosenBag) return { ok: false, reason: 'no free slot in any bag' };
...
export async function openPackages(client, currentGrid, opts = {}) {
  ...
  for (const pkg of packages) {
    ...
    try {
      ...
      log.warn(`package ${pkg.packageId} move failed: ${e.message}`);
      skipped++;
    }
  }
...
export function openHealPackages(client, currentGrid, maxToOpen = Infinity) {
